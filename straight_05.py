...
while True:
    ...
    if now_moment['distance'] > initial_data['catchDistance']:
        ...
        s = abs(aimx - catcherx)
        h = abs(aimy - catchery)
        ds = -abs(aim_speed * cos(radians(now_moment['aimPhi'])) - catcher_speed)
        dphi = abs(aim_speed)/(h * (1 + (s/h) ** 2))
        if ds < 0:
            ang = now_moment['aimPhi'] - 0.5 * dphi * deltas / ds
        else: 
            ang = now_moment['aimPhi'] + 0.5 * dphi * deltas / ds                                                     
        points = circle_straigth(50000, 0, 50000 + now_moment['aimR'] * cos(radians(ang)), 0 + now_moment['aimR'] * sin(radians(ang)), catcherx, catchery, catcher_speed)    
...
while aim_x > 50000 + 2 * aim_speed_x:
    now_moment = {'moment' : -2, 'aimPhi' : 0, 'aimR' : 0,
                  'catcherPhi' : 0, 'catcherRho' : 0, 'distance' : 0}
